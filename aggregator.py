        ...
        while True:
            ...
            for res in results:
                ...
                for item in res:
                    ...
                    # Фильтр по возрасту — только новости за последние 24 часа
                    try:
                        pub = item.pub_date
                        ...
                        if age_hours > 24:
                            log.debug("SKIP old (%.0fh): %s", age_hours, item.headline[:50])
                            continue
                    except Exception:
                        pass  # если дата не распарсилась — пропускаем всё равно

                    await self._queue.put(item)
                    new_count += 1
                    log.info("NEW  [%s] +%.0fh %s", item.source, age_hours, item.headline[:65])
