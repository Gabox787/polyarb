    ...
    try:
        ...
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=8)) as resp:
            if resp.status != 200:
                ...
                log.warning("5m check HTTP %d for market_id=%s url=%s body=%s",
                            resp.status, pos.market_id, url, body[:200])
            ...
            log.info("5m check market_id=%s closed=%s active=%s question=%s",
                      pos.market_id, closed, active, question[:50])
            ...
            log.info("5m raw fields: outcomePrices(%s)=%r outcomes(%s)=%r",
                      type(raw_prices).__name__, raw_prices,
                      type(raw_outcomes).__name__, raw_outcomes)

            try:
                outcome_prices = json.loads(raw_prices) if isinstance(raw_prices, str) else raw_prices
            except Exception as e:
                log.warning("Failed to parse outcomePrices: %s", e)
                outcome_prices = []
            try:
                outcomes = json.loads(raw_outcomes) if isinstance(raw_outcomes, str) else raw_outcomes
            except Exception as e:
                log.warning("Failed to parse outcomes: %s", e)
                outcomes = []

            log.info("5m parsed: outcomes=%s prices=%s", outcomes, outcome_prices)

            if not outcome_prices or not outcomes or len(outcome_prices) < 2:
                log.warning("5m market closed but no usable outcome data — market_id=%s", pos.market_id)
            ...
            log.info("5m RESULT: side=%s our_idx=%d price=%s outcome=%s",
                      pos.side, our_idx, price, outcome_name)

            return True, price, outcome_name
    except Exception as e:
        log.warning("5m result check EXCEPTION for market_id=%s: %s", pos.market_id, e)
        return False, pos.entry_price, ""
